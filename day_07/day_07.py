...
def compute_amplifier_output(program, phase_setting):
    ...
    for phase in phase_setting:
        ...
        output = intcode_computer.execute_intcode(current_program, [phase, output])[1][-1]
    return output
...
def max_thruster_signal_with_feedback(program):
    ...
    for phase_setting in itertools.permutations(range(5,10)):
        ...
        while state_amp_e[0] != True:
            ...
            state_amp_b[2] += output
            state_amp_b[1], output, state_amp_b[3], state_amp_b[0] = intcode_computer.execute_intcode(state_amp_b[1], state_amp_b[2], state_amp_b[3])
            
            state_amp_c[2] += output
            state_amp_c[1], output, state_amp_c[3], state_amp_c[0] = intcode_computer.execute_intcode(state_amp_c[1], state_amp_c[2], state_amp_c[3])
           
            state_amp_d[2] += output
            state_amp_d[1], output, state_amp_d[3], state_amp_d[0] = intcode_computer.execute_intcode(state_amp_d[1], state_amp_d[2], state_amp_d[3])
           
            state_amp_e[2] += output
            state_amp_e[1], output, state_amp_e[3], state_amp_e[0] = intcode_computer.execute_intcode(state_amp_e[1], state_amp_e[2], state_amp_e[3])
...
if __name__ == "__main__":
    ...
    print("Max thruster signal: ", max_thruster_signal(list(program)))
    print("Max thruster signal with feedback: ", max_thruster_signal_with_feedback(list(program))[0])